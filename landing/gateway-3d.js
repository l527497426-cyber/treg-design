...
export async function mountGateway(host,{studio=false}={}){
 ...
 let font;try{font=new FontLoader().parse(await (await fetch('./vendor/three/helvetiker_regular.typeface.json')).json());}catch{}
...
if(landing)mountGateway(landing).then(api=>{window.tregModel=api;}).catch(error=>{console.warn('3D gateway unavailable; no legacy illustration is loaded.',error);});
